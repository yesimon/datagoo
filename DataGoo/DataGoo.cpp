...
String mobileNumber = "16503845765"; //phone number to text status updates to
...
void setup()
{
  ...
  if (cellFile) {
    while (cellFile.available()) {
      mobileNumber += (char) cellFile.read();
    }
    mobileNumber.trim(); //get rid of whitespace
    Serial.println("mobile num is: " + mobileNumber);
  } else {
    Serial.println("Please make a file called CELL.TXT on the SD card containing the phone number you would like power logger to text with daily stats");
  }*/
...
void loop() {
  ...
  if (timeDiff > 10) { //24 hrs * 60 mins * 60 secs = 86400 secs/day
    ...
    if (logFile) {
      ...
      logFile.println("millis is " + String(now));
      logFile.close();
      lastLoggedTime = now;
    } else {
  ...
  if (numTextsSent < 2) {
    ...
    cellReadLine();
    //if (inputString.startsWith("+CCLK:")) {
